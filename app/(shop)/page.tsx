...
import { ArrowRight, ShoppingCart, MapPin, CheckCircle, Truck, Star } from 'lucide-react'
...
export default function ShopHomePage() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="bg-gradient-to-br from-green-700 via-green-800 to-green-900 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 text-green-100 text-sm font-medium mb-6">
            <CheckCircle className="w-4 h-4" /> Verified Liberian Farmers · No Middlemen
          </div>
          <h1 className="text-4xl sm:text-5xl font-extrabold leading-tight mb-4">
            Fresh Produce<br />
            <span className="text-green-300">Delivered to Your Door</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/shop/products" className="flex items-center gap-2 px-8 py-3.5 rounded-full bg-white text-green-800 font-bold text-base hover:bg-green-50 transition shadow-lg">
          ...
          <div className="flex items-center justify-center gap-6 mt-10 text-sm text-green-300">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {CATEGORIES.map((cat) => (
            <Link
              key={cat.label}
              href="/shop/products"
              className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-white border border-gray-100 hover:border-green-300 hover:shadow-md transition group text-center"
            >
              <span className="text-4xl group-hover:scale-110 transition-transform">{cat.emoji}</span>
              <span className="text-xs font-semibold text-gray-700 group-hover:text-green-700">{cat.label}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/shop/products" className="text-sm text-green-700 font-semibold hover:underline flex items-center gap-1">View all <ArrowRight className="w-4 h-4" /></Link>
        </div>
      ...
      <section className="bg-white border-y border-gray-100 py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-500 mb-10">Order fresh produce from your phone in 3 easy steps</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {HOW_IT_WORKS.map((s) => (
              <div key={s.step} className="flex flex-col items-center">
                <div className="w-14 h-14 rounded-2xl bg-green-50 border border-green-100 flex items-center justify-center text-3xl mb-4">{s.icon}</div>
                <div className="w-6 h-6 rounded-full bg-green-600 text-white text-xs font-bold flex items-center justify-center mb-3">{s.step}</div>
                <h3 className="font-bold text-gray-900 mb-2">{s.title}</h3>
                <p className="text-sm text-gray-500 leading-relaxed">{s.desc}</p>
              </div>
            ))}
          </div>
          <div className="mt-10 inline-flex items-center gap-3 px-5 py-3 rounded-xl bg-blue-50 border border-blue-200 text-sm text-blue-700">
      ...
      <section className="bg-green-700 text-white py-14">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-green-200 mb-7">Join AgriHub Liberia and reach buyers across all 15 counties. Free to list. Pay when you sell.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/auth/register" className="px-7 py-3 rounded-full bg-white text-green-800 font-bold hover:bg-green-50 transition">
              Register as Farmer
            </Link>
            <Link href="/shop/auth/register" className="px-7 py-3 rounded-full border-2 border-white/40 text-white font-semibold hover:bg-white/10 transition">
