...
export default function ProductCard({ listing }: Props) {
  ...
  return (
    <div className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-green-200 transition-all duration-200">
      <Link href={`/shop/products/${listing.id}`}>
        <div className="relative h-48 overflow-hidden bg-gray-50">
          ...
          {listing.qualityVerified && (
            <span className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 rounded-full bg-green-500 text-white text-xs font-semibold">
              <CheckCircle className="w-3 h-3" /> Verified
            </span>
          )}
          <button
            onClick={(e) => { e.preventDefault(); toggleSaved(listing.id) }}
            className={`absolute top-3 left-3 w-7 h-7 rounded-full flex items-center justify-center text-sm transition ${isSaved ? 'bg-red-500 text-white' : 'bg-white/80 text-gray-500 hover:bg-white'}`}
          >
            {isSaved ? '♥' : '♡'}
          </button>
        </div>
        ...
        <div className="p-4">
          <div className="flex items-start justify-between gap-2">
            <div>
              <h3 className="font-bold text-gray-900 text-sm group-hover:text-green-700 transition-colors line-clamp-1">
            ...
            <div className="text-right flex-shrink-0">
              <p className="font-bold text-green-700 text-base">L${listing.pricePerUnitLRD}</p>
          ...
          <div className="flex items-center justify-between mt-1 text-xs text-gray-400">
            ...
            <span className="capitalize text-gray-500">{listing.quality}</span>
          </div>
        </div>
      </Link>
      ...
      <div className="px-4 pb-4">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold transition"
        >
